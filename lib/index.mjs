...
function Harness ({
   ...
   */
  const Context = ({ suite, tests }) => ᐅeffect(ᐅdo([
    TestRunner.create,
    set.value.mut(`run_test`),
  ]))({
    suite,
    tests,
    reporter,
   ...
   */
  function suite (description, test_definitions) {
    const tests  = enumerable_entries(test_definitions)
    const context = Context({ suite, tests })
    context.next_reporter(context.reporter(intercept.Message({
      prefix  : 'sisyphus',
      type    : 'suite:prerun',
      payload : { description, length: len(context.tests) },
      sender  : context.suite,
    })))
    const result = map_indexed(context.run_test)(tests)
    context.next_reporter(context.reporter(intercept.Message({
      prefix  : 'sisyphus',
      type    : 'suite:result',
      payload : { result, description, length: len(context.tests) },
      sender  : context.suite,
    })))
    return result
  }
