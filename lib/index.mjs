...
function streaming_values (assertions, reporter) {
  const intercept = interceptor({
    argument : ᐅeffect(value => reporter({ type: `value`, value })),
  })
...
function make_suite (base_assertions, reporter) {
  return function suite (description, tests) {
    const enabled_tests = enumerable_entries(tests)
        , length        = len(enabled_tests)
        , assertions    = merge({ suite })(base_assertions)
    reporter({ type: 'suite', description, length })
    return map(tester(assertions, reporter, length))(enabled_tests)
  }
}

function tester (assertions, reporter, length) {
  return function run_test ([ description, test ], index) {
...
function processor ({
  ...
  const test_receiver = message => {
    ...
    switch (message.type) {
      case 'value'       : return apply_receiver(value)
