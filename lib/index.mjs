...
const eq = a => b => {
  ...
  const [ a_keys, a_descs ] = disinterlace(sort(pairs_by_0)(get.all.properties(a)))
      , [ b_keys, b_descs ] = disinterlace(sort(pairs_by_0)(get.all.properties(b)))
...
function streaming_values (t, stream) {
  const interceptor = intercept({
    argument : ᐅeffect(value => stream({ type: `value`, value })),
  })
  return map_properties(([ name, descriptor ]) => {
    return [ name, fallible.unwrap(update(`value`)(interceptor)(descriptor)) ]
  })(t)
}

function intercept1 ({ before=id, after=id }) {
  return f => ᐅ([ before, f, after ])
}

function intercept ({ argument=id, result=id }) {
  const is_function = reflex.type(`function`)
  // NOTE(jordan): thunk it out! otherwise infinite recursion...
  return f => intercept1({
    before: argument,
    after: ᐅif(is_function)(intercept({ argument, result }))(result),
  })(f)
}

function make_suite (base_t, stream) {
  return function suite (description, assertions) {
    const tests  = enumerable_entries(assertions)
        , length = len(tests)
        , t = merge({ suite })(base_t)
    stream({ type: 'suite', description, length })
    return map(tester(t, stream, length))(tests)
  }
}

function tester (t, stream, length) {
  return function test ([ description, assertion ], index) {
