...
import * as intercept from './intercept.mjs'
...
const simple = or([ not(is_value), not(reflex.type.object) ])
  ...
  create (context) {
    return function run_test (index) {
      return test => {
        ...
        context.assign_reporter(context.assertions)
        const result = test(context.assertions)
        const report = context.recall_reporter(context.assertions)
        /* NOTE(jordan): When the report is not a result from a subsuite,
         * send a test result message; otherwise, do nothing.
         */
        if (
          not(and([
