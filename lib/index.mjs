...
function Harness ({
  ...
  const Context = ({ depth, suite, tests }) => ᐅeffect(ᐅ([
    ...
    suite (description, test_definitions) {
      return suite(description, test_definitions, { depth: depth + 1 })
    },
  })
  ...
  function suite (
    ...
    {
      depth   = 1,
      tests   = enumerable_entries(test_definitions),
      context = Context({ suite, tests, depth }),
    } = {},
  ) {
    ...
    context.next_reporter(context.reporter(intercept.Message({
      prefix  : 'sisyphus:suite',
      type    : 'result',
      payload : { result, depth, length, description, is_subsuite },
      sender  : context.suite,
    })))
    return result
  }
